...
FACECAM_FOLDER = os.path.join(os.path.dirname(os.path.abspath(__file__)), "overlays/")
...
class ClipPrepAbstract:
    ...
    def _choose_random_song(self, music_folder: str) -> str:
        ...
        """
        song_name = os.path.join(music_folder, random.choice(os.listdir(music_folder)))
    ...
    def _choose_random_facecam_clip(self, facecam_folder: str) -> str:
        ...
        if random.randint(1, 6) == 7:
            ...
            self._face_cam_start_time = float(
                re.search(r"(\d+\$\d+)", face_cam_name).group(1).replace("$", ".")
            )
...
class ClipPrepValorant(ClipPrepAbstract):
    ...
    def _find_highlight_time(self, file: str) -> list[float]:
        ...
        """
        video_clip = moviepy.editor.VideoFileClip(file).crop(
            self._left_crop, self._top_crop, self._right_crop, self._bottom_crop
        )
        highlights = []
        time_interval = 0
        while time_interval < video_clip.duration:
            colour = cv2.cvtColor(
                video_clip.get_frame(time_interval), cv2.COLOR_RGB2BGR
            )
            grayscale = cv2.cvtColor(colour, cv2.COLOR_BGR2GRAY)
            circles = cv2.HoughCircles(
                grayscale, cv2.HOUGH_GRADIENT, 1.5, 100, minRadius=40, maxRadius=60
            )
    ...
    def _find_highlight_time(self, file: str) -> list[float]:
        ...
        while time_interval < video_clip.duration:
            hsv_frame = cv2.cvtColor(
                video_clip.get_frame(time_interval), cv2.COLOR_RGB2HSV
            )
            mask = cv2.inRange(
                hsv_frame, yellow_hsv_lower_bound, yellow_hsv_upper_bound
            )
