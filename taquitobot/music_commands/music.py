...
class MusicCommands(commands.Cog):
    ...
    async def play(self, ctx: commands.Context) -> None:
        ...
        if not self._is_connected(guild=ctx.guild):
            await self._connect_to_call(ctx=ctx)
    ...
    async def _connect_to_call(self, ctx: commands.context.Context) -> None:
        """
    ...
    def _play_song(self, ctx: commands.Context) -> None:
        ...
        voice_channel.play(
            discord.FFmpegOpusAudio(song_stream),
    ...
    def _get_song_from_yt(song_name) -> tuple[str, str]:
        ...
        with yt_dlp.YoutubeDL(ytdl_opts) as yt:
            song_info = yt.extract_info(f"ytsearch:{song_name}", download=False)
